<?php
    ...
    function calculate_nilai_akhir($uts, $uas, $tugas)
    {
        return ($uts * 0.4) + ($uas * 0.4) + ($tugas * 0.2);
    }
    ...
    function handle_submit()
    {
        ...
        if(!is_valid_nilai($uts) || !is_valid_nilai($uas) || !is_valid_nilai($tugas))
            return 'Nilai tidak valid';

        $result = calculate_nilai_akhir($uts, $uas, $tugas);
        return 'Nilai akhir anda adalah: ' . $result . ' dengan kategori ' . categorize($result);
    }

    if($_SERVER['REQUEST_METHOD'] === 'POST')
    {
        $message = handle_submit();
